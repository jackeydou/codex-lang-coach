import{mkdirSync as A}from"node:fs";import{Buffer as T}from"node:buffer";import{homedir as R}from"node:os";import{dirname as I,join as C}from"node:path";import{randomUUID as b}from"node:crypto";import{DatabaseSync as x}from"node:sqlite";function N(n,e=new Date){let r=a=>a.toISOString().slice(0,10),t=new Map;for(let a of n){let i=a.createdAt.slice(0,10);t.set(i,(t.get(i)??0)+1)}let s=a=>Array.from({length:a},(i,h)=>{let u=new Date(e);u.setUTCHours(0,0,0,0),u.setUTCDate(u.getUTCDate()-(a-1-h));let _=r(u);return{date:_,count:t.get(_)??0}}),c=s(7),o=0;for(let a=0;a<366;a+=1){let i=new Date(e);if(i.setUTCDate(i.getUTCDate()-a),(t.get(r(i))??0)>0)o+=1;else if(a>0||t.size>0)break}let d=new Map,g=new Map,l={native:0,target:0,mixed:0,other:0};for(let a of n){l[a.inputLanguage]+=1;for(let i of a.corrections)d.set(i.category,(d.get(i.category)??0)+1);for(let i of a.patterns){let h=i.pattern.trim().toLocaleLowerCase(),u=g.get(h)??{explanation:i.explanation,count:0};u.count+=1,g.set(h,u)}}return{totalNotes:n.length,notesThisWeek:c.reduce((a,i)=>a+i.count,0),activeDays:t.size,currentStreak:o,weeklyActivity:c,activity90Days:s(90),categoryCounts:[...d.entries()].map(([a,i])=>({category:a,count:i})).sort((a,i)=>i.count-a.count),recurringPatterns:[...g.entries()].map(([a,i])=>({pattern:a,...i})).sort((a,i)=>i.count-a.count).slice(0,50),languageUse:{...l,targetShare:l.native+l.target>0?Math.round(l.target/(l.native+l.target)*100):0}}}function U(n=process.env){return n.LANGUAGE_COACH_DB_PATH||C(R(),".language-coach","language-coach.sqlite")}function O(n){if(n)try{let e=JSON.parse(T.from(n,"base64url").toString("utf8"));return e.createdAt&&e.id?{createdAt:e.createdAt,id:e.id}:void 0}catch{return}}function D(n){return T.from(JSON.stringify({createdAt:n.createdAt,id:n.id})).toString("base64url")}function E(){return new Date().toISOString()}function f(n,e){if(typeof n!="string")return e;try{return JSON.parse(n)}catch{return e}}function L(n){return n.replace(/\/$/,"")}function p(n){return{id:n.id,turnId:n.turn_id??void 0,inputLanguage:n.input_language||"other",originalExpression:n.original_expression,polishedExpression:n.polished_expression,corrections:f(n.corrections_json,[]),patterns:f(n.patterns_json,[]),examples:f(n.examples_json,[]),nativeLanguage:n.native_language,targetLanguage:n.target_language,createdAt:n.created_at}}var m=class{database;constructor(e=U()){A(I(e),{recursive:!0}),this.database=new x(e),this.database.exec("PRAGMA journal_mode = WAL; PRAGMA foreign_keys = ON;"),this.migrate()}migrate(){this.database.exec(`
    ...
    `),this.database.prepare("PRAGMA table_info(profile)").all().some(o=>o.name==="sync_revision")||this.database.exec("ALTER TABLE profile ADD COLUMN sync_revision INTEGER NOT NULL DEFAULT 0");let r=this.database.prepare("PRAGMA table_info(learning_notes)").all();r.some(o=>o.name==="input_language")||this.database.exec("ALTER TABLE learning_notes ADD COLUMN input_language TEXT NOT NULL DEFAULT 'other'"),r.some(o=>o.name==="sync_revision")||this.database.exec("ALTER TABLE learning_notes ADD COLUMN sync_revision INTEGER NOT NULL DEFAULT 0"),this.database.prepare("PRAGMA table_info(deleted_learning_notes)").all().some(o=>o.name==="sync_revision")||this.database.exec("ALTER TABLE deleted_learning_notes ADD COLUMN sync_revision INTEGER NOT NULL DEFAULT 0");let s=E();this.database.prepare(`INSERT OR IGNORE INTO profile
      ...
      ON CONFLICT(remote_url, user_id) DO UPDATE SET
        ...
        last_synced_at = excluded.last_synced_at`).run(L(e),r,t,s)}close(){this.database.close()}};import*as P from"node:tls";function k(){let n=P;if(!n.getCACertificates||!n.setDefaultCACertificates)return;let e=new Set([...n.getCACertificates("default"),...n.getCACertificates("system")]);n.setDefaultCACertificates([...e])}k();async function S(){let n="";for await(let e of process.stdin)n+=e;return n?JSON.parse(n):{}}var y=await S(),v=new m,w=v.getProfile();v.close();var M=typeof y.loop_count=="number"?y.loop_count:0,F=typeof y.status=="string"?y.status:"";!w.coachEnabled||F!=="completed"||M>0?process.stdout.write("{}"):process.stdout.write(JSON.stringify({followup_message:"Before finishing, decide whether the latest user message has meaningful language-learning value. If you have not already handled it, save a note with the Language Coach MCP tool save_learning_note only when the expression contains a meaningful error, unnatural or contextually inappropriate wording, or a genuinely useful reusable pattern. Do not save a duplicate or save anything when the expression is already natural, correct, and appropriate. Never save unrelated task content. Then return the response without discussing this internal decision."}));
