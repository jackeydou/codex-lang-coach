import{mkdirSync as v}from"node:fs";import{homedir as w}from"node:os";import{dirname as C,join as A}from"node:path";import{randomUUID as S}from"node:crypto";import{DatabaseSync as I}from"node:sqlite";function P(r=process.env){return r.LANGUAGE_COACH_DB_PATH||A(w(),".language-coach","language-coach.sqlite")}function f(){return new Date().toISOString()}function L(r,e){if(typeof r!="string")return e;try{return JSON.parse(r)}catch{return e}}function T(r){return{id:r.id,turnId:r.turn_id??void 0,inputLanguage:r.input_language||"other",originalExpression:r.original_expression,polishedExpression:r.polished_expression,corrections:L(r.corrections_json,[]),patterns:L(r.patterns_json,[]),examples:L(r.examples_json,[]),nativeLanguage:r.native_language,targetLanguage:r.target_language,createdAt:r.created_at}}var p=class{database;constructor(e=P()){v(C(e),{recursive:!0}),this.database=new I(e),this.database.exec("PRAGMA journal_mode = WAL; PRAGMA foreign_keys = ON;"),this.migrate()}migrate(){this.database.exec(`
    ...
    `),this.database.prepare("PRAGMA table_info(learning_notes)").all().some(t=>t.name==="input_language")||this.database.exec("ALTER TABLE learning_notes ADD COLUMN input_language TEXT NOT NULL DEFAULT 'other'");let o=f();this.database.prepare(`INSERT OR IGNORE INTO profile
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(t.id,t.turnId??null,t.inputLanguage,t.originalExpression,t.polishedExpression,JSON.stringify(t.corrections),JSON.stringify(t.patterns),JSON.stringify(t.examples),t.nativeLanguage,t.targetLanguage,t.createdAt),t}hasNoteForTurn(e){return!!this.database.prepare("SELECT 1 FROM learning_notes WHERE turn_id = ?").get(e)}listNotes(e=100){let o=Math.max(1,Math.min(500,Math.trunc(e)));return this.database.prepare("SELECT * FROM learning_notes ORDER BY created_at DESC LIMIT ?").all(o).map(T)}deleteNote(e){return this.database.prepare("DELETE FROM learning_notes WHERE id = ?").run(e).changes>0}getProgress(){let e=this.listNotes(500),o=new Date,t=n=>n.toISOString().slice(0,10),i=new Map;for(let n of e){let a=n.createdAt.slice(0,10);i.set(a,(i.get(a)??0)+1)}let c=Array.from({length:7},(n,a)=>{let g=new Date(o);g.setUTCHours(0,0,0,0),g.setUTCDate(g.getUTCDate()-(6-a));let u=t(g);return{date:u,count:i.get(u)??0}}),E=0;for(let n=0;n<366;n+=1){let a=new Date(o);if(a.setUTCDate(a.getUTCDate()-n),(i.get(t(a))??0)>0)E+=1;else if(n>0||i.size>0)break}let h=new Map,m=new Map,s={native:0,target:0,mixed:0,other:0};for(let n of e){s[n.inputLanguage]+=1;for(let a of n.corrections)h.set(a.category,(h.get(a.category)??0)+1);for(let a of n.patterns){let g=a.pattern.trim().toLocaleLowerCase(),u=m.get(g)??{explanation:a.explanation,count:0};u.count+=1,m.set(g,u)}}return{totalNotes:e.length,notesThisWeek:c.reduce((n,a)=>n+a.count,0),activeDays:i.size,currentStreak:E,weeklyActivity:c,categoryCounts:[...h.entries()].map(([n,a])=>({category:n,count:a})).sort((n,a)=>a.count-n.count),recurringPatterns:[...m.entries()].map(([n,a])=>({pattern:n,...a})).sort((n,a)=>a.count-n.count).slice(0,8),languageUse:{...s,targetShare:s.native+s.target>0?Math.round(s.target/(s.native+s.target)*100):0}}}getDashboardData(e=100){return{profile:this.getProfile(),notes:this.listNotes(e),progress:this.getProgress()}}close(){this.database.close()}};async function y(){let r="";for await(let e of process.stdin)r+=e;return r?JSON.parse(r):{}}var _=await y(),N=new p,b=N.getProfile();N.close();b.coachEnabled||process.exit(0);var{nativeLanguage:d,targetLanguage:l}=b,O=typeof _.turn_id=="string"?_.turn_id:"",x=l.trim().toLowerCase()==="english"?"natural, contemporary American English":`natural, contemporary ${l}`,D=`# Language coach mode
...
1. Aim for ${x}: the way people normally speak and write in daily life, not stiff or textbook-style language. Preserve the user's intended meaning, tone, and level of politeness.
2. If the user writes in ${l}, check grammar, spelling, collocations, word choice, tone, and contextual appropriateness. Briefly identify meaningful problems, then rewrite the message the way a native speaker would naturally express it in the same situation. Fix awkward phrasing even when it is technically grammatical.
3. If the user writes mainly in ${d}, translate the intended meaning into ${x}. Translate the message as a whole instead of following the original word order or sentence structure.
...
6. Highlight reusable grammar patterns, sentence structures, collocations, or phrases. Explain them briefly in ${d} when that helps the learner.
...
- Do not mention the persistence call unless it fails or the user asks about storage.`;process.stdout.write(JSON.stringify({hookSpecificOutput:{hookEventName:"UserPromptSubmit",additionalContext:D}}));
